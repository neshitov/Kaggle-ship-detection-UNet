...
def detection_train(epochs):
    ...
    active_parameters = [
        par for par in rn.parameters() if par.requires_grad]
...
def DiceLoss(input, target):
    ...
    iflat = input.view(-1)
    tflat = target.view(-1)
    dl = 1-(iflat*tflat.sum()+eps)/(iflat.sum()+tflat.sum()+eps) - \
        (((1-iflat)*(1-tflat).sum()+eps)/((1-iflat).sum()+(1-tflat).sum()+eps))
    return dl
...
class Loss(nn.Module):
    ...
    def forward(self, input, target):
        loss = self.alpha*self.focal(input, target) - \
            torch.log(DiceLoss(input, target))
        return loss.mean()
